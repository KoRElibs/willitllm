...
//              dequantization overhead. Treat as ±2× rough estimate.
//
...
//              with context length and GPU architecture. Treat as ±2× rough estimate.
//
...
const QUANT_INFO = {
  ...
  'Q3_K_M':  { speed:  9, quality:  4, gen_eff: [0.40, 0.78], prefill_eff: [0.06, 0.30], summary: 'Small step up from Q3_K_S; reasonable if VRAM is tight.' },
  'Q3_K_L':  { speed:  8, quality:  4, gen_eff: [0.40, 0.78], prefill_eff: [0.06, 0.30], summary: 'Best standard 3-bit option; worth it over S/M when you have room.' },
  'Q4_0':    { speed:  8, quality:  5, gen_eff: [0.43, 0.82], prefill_eff: [0.07, 0.33], summary: 'Older 4-bit method; Q4_K_M gives better results for the same VRAM.' },
  'Q4_1':    { speed:  7, quality:  5, gen_eff: [0.43, 0.82], prefill_eff: [0.07, 0.33], summary: 'Marginal step up from Q4_0; Q4_K_M is nearly always the better pick.' },
  'Q4_K_S':  { speed:  7, quality:  6, gen_eff: [0.43, 0.82], prefill_eff: [0.07, 0.33], summary: 'Slightly smaller than Q4_K_M with similar quality; good when VRAM is close.' },
  'Q4_K_M':  { speed:  7, quality:  6, gen_eff: [0.43, 0.82], prefill_eff: [0.07, 0.33], summary: 'The most popular choice — best size-to-quality ratio at 4-bit.' },
  'Q5_0':    { speed:  6, quality:  7, gen_eff: [0.45, 0.85], prefill_eff: [0.07, 0.33], summary: 'Older 5-bit method; Q5_K_M is a better use of the same VRAM.' },
  'Q5_1':    { speed:  6, quality:  7, gen_eff: [0.45, 0.85], prefill_eff: [0.07, 0.33], summary: 'Marginal step up from Q5_0; Q5_K_M is nearly always the better pick.' },
  'Q5_K_S':  { speed:  6, quality:  7, gen_eff: [0.45, 0.85], prefill_eff: [0.07, 0.33], summary: 'Good 5-bit option when you need to save a little VRAM over Q5_K_M.' },
  'Q5_K_M':  { speed:  5, quality:  8, gen_eff: [0.45, 0.85], prefill_eff: [0.07, 0.33], summary: 'Near-original quality; use this when Q4 feels insufficient.' },
  'Q6_K':    { speed:  4, quality:  9, gen_eff: [0.48, 0.88], prefill_eff: [0.07, 0.35], summary: 'Barely distinguishable from full precision; for when quality cannot be compromised.' },
  'Q8_0':    { speed:  3, quality:  9, gen_eff: [0.50, 0.90], prefill_eff: [0.08, 0.38], summary: 'Effectively lossless; great for benchmarking or ruling out compression effects.' },
  // Full precision
  'F16':     { speed:  1, quality: 10, gen_eff: [0.55, 0.92], prefill_eff: [0.10, 0.45], summary: 'Full 16-bit precision; use when VRAM is not a constraint.' },
  'FP16':    { speed:  1, quality: 10, gen_eff: [0.55, 0.92], prefill_eff: [0.10, 0.45], summary: 'Same as F16; full precision baseline.' },
  'BF16':    { speed:  1, quality: 10, gen_eff: [0.55, 0.92], prefill_eff: [0.10, 0.45], summary: 'Brain float 16-bit; same quality as F16, better suited to modern hardware.' },
  'F32':     { speed:  1, quality: 10, gen_eff: [0.55, 0.92], prefill_eff: [0.10, 0.45], summary: 'Full 32-bit float; maximum precision, rarely needed for inference.' },
};
